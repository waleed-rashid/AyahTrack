...
router.post("/verify-email", async (req, res) => {
    ...
    const isValidCode = await bcrypt_1.default.compare(code, verification.codeHash);
    if (!isValidCode) {
    ...
    const emailVerificationToken = jsonwebtoken_1.default.sign({ email, purpose: "email-verification" }, getJwtSecret(), { expiresIn: EMAIL_VERIFICATION_TOKEN_EXPIRATION });
    res.json({
...
router.post("/signup", async (req, res) => {
    const { name, email, password, memorizedJuzCount = 0, memorizedJuzList = [], memorizedSurahList = [], currentJuz, currentSurah, currentAyah, averageSabaqPages = 0.5, averageSabaqParaPages = 3, averageRevisionJuz = 0.25, emailVerificationToken, } = req.body;
    const normalizedEmail = normalizeEmail(email);
    try {
        const verificationPayload = jsonwebtoken_1.default.verify(String(emailVerificationToken || ""), getJwtSecret());
        if (verificationPayload.purpose !== "email-verification" ||
    ...
    const token = jsonwebtoken_1.default.sign({ userId: user.id }, getJwtSecret(), { expiresIn: "7d" });
