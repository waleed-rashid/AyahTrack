...
type DeviceSessionSummary = {
  sabaqSeconds: number;
  sabaqParaSeconds: number;
  revisionSeconds: number;
};

const emptySessionSummary = (): DeviceSessionSummary => ({
  sabaqSeconds: 0,
  sabaqParaSeconds: 0,
  revisionSeconds: 0,
});
...
export const attachDeviceSessionSummaries = async <T extends EntryWithDate>({
  prisma,
  userId,
  entries,
}: {
  ...
  const sessionSummaries = sessions.reduce<Record<string, DeviceSessionSummary>>(
    (summaries, session) => {
